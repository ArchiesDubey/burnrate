...
| **Grok** | official | Grok Build's weekly credits from the same billing endpoint the CLI uses, with the session in `~/.grok/auth.json` — run `grok login` once and the ring appears. |
...
Every reading is borrowed from a credential or session a tool on your Mac
already holds — install and sign in to any of them, and its ring appears.
Switching a provider off stops its credential being read at all and forgets
the readings taken from it; it does not sign you out of the tool that owns
...
Auto-update is **disabled** in this codebase. [Sparkle](https://sparkle-project.org)
is still wired in — Settings keeps its controls — but it checks nothing until
three things exist: a download feed you control, an `SUPublicEDKey` generated by
`xcrun sparkle-generate-keys` in `project.yml`, and the two `SU*` switches
flipped back on there. Until then the app never phones home for updates, by
design.
...
Run with `BURNRATE_DEMO=1` to see fixed sample data instead of live readings.
...
```sh
/usr/bin/log stream --predicate 'subsystem == "com.burnrate.burnrate"' --level debug
```
...
Burnrate is a fork of [Codenotch](https://github.com/vinzdg/codenotch) by
Vinz, rebranded and extended. The upstream project and its contributors —
including Mintaras Grinius, maheshauti96, canberkys, TrapstarKS, Gabriel
Rodriguez Diaz, Mohammad Albarham, Muhammad Hamza, and Paweł Puzio — hold the
copyright on their work, which is used here under the MIT license. Their
copyright notice is preserved in [LICENSE](LICENSE) as that license requires.
