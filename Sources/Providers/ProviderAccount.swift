...
///
/// Worth showing plainly, because Burnrate never signs in — it borrows a
...
///
/// Burnrate cannot sign anyone in — it reads a credential the owning tool
...
enum SignInRoute: Equatable {
    ...
    /// The only case where Burnrate genuinely signs anyone in or out.
    ...
    var signOutCaveat: String {
        switch self {
        case .modal(let name):
            return "Signs out of \(name) — the session belongs to Burnrate."
